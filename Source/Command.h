...
class ATL_NO_VTABLE CCUBRIDCommand :
	...
	public ICommandTextImpl<CCUBRIDCommand>,
...
public:
	...
	DBCOUNTITEM			m_cParams;
	DBCOUNTITEM			m_cParamsInQuery;
	DBPARAMINFO*		m_pParamInfo;

	CAtlArray<int> m_gOIDIndex; // start from 1
	CAtlArray<CStringA> m_gOID;

	int			m_hReq; //CCI request handle; It is obtained from PrepareCommand(...)
