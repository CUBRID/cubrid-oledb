...
EXIT:
			...
			const GUID* pGuid = NULL)
		{
			...
			for(ulSet = 0; ulSet < cSets; ulSet++)
			{
				...
				// Calculate the number of property nodes needed for this property set.
				if( cPropertySets == 0 )
				{
					ULONG ulTempSet;
					if (pGuid != NULL)
					...
					for(ul = 0; ul < m_cPropSetDex; ul++)
					{
						...
						else
						{
							...
							for( ulProp = 0; ulProp < cIterations; ulProp++, ulNext++ )
							{
								...
								if( GetUPropInfoPtr(m_rgiPropSetDex[ul], pCurPropInfo->dwPropertyID, &pUPropInfo) == S_OK )
								{
									// If the ppDescBuffer pointer was not NULL, then
									// we need supply description of the properties
									if( ppDescBuffer )
...
EXIT:
			...
			const ULONG cSelectProps = 1, const GUID* const ppGuid[] = NULL)
		{
			...
			{
				ATLTRACE(atlTraceDBProvider, 0, "Could not allocate DBPROPSET array for GetProperties\n");
				return E_OUTOFMEMORY;
			}
				...
				if( pProp )
				{
					...
					for(ulProp = 0; ulProp < cPropertyIDs; ulProp++)
					{
						...
						else
						{
							...
							if( GetIndexofPropIdinPropSet(ulCurSet, pCurProp->dwPropertyID, &ulCurProp) == S_OK)
							{
								// Supported
								pUPropInfo = &(m_pUPropSet[ulCurSet].pUPropInfo[ulCurProp]);
								ATLASSERT( pUPropInfo );
								...
								if( pUPropInfo->dwFlags & (DBPROPFLAGS_WRITE | DBPROPFLAGS_CHANGE) )
								{
									pUPropVal = &(m_pUProp[ulCurSet].pUPropVal[GetUPropValIndex(ulCurSet, pCurProp->dwPropertyID)]);
									ATLASSERT( pUPropVal );
								...
								{
									GetDefaultValue(ulCurSet, pUPropInfo->dwPropId, &(pCurProp->dwOptions), &(pCurProp->vValue));
								}
		...
		BOOL IsPropSet(const GUID* pguidPropSet, DBPROPID dwPropId)
		{
			...
			if( GetIndexofPropSet(pguidPropSet, &iPropSet) == S_OK )
			{
				if( GetIndexofPropIdinPropSet(iPropSet, dwPropId, &iPropId) == S_OK )
				{
					...
					{
						hr = GetDefaultValue(iPropSet, dwPropId, &dwOptions, &vValue);
					}
	...
	public:
			...
			const VARIANT rgRestrictions[])
		{
			...
			const VARTYPE rgRestrictionTypes[3][4] = {
				...
				{ VT_UI2, VT_BOOL, VT_EMPTY, VT_EMPTY }
			...
			*prgInfo = (DBCOLUMNINFO*)CoTaskMemAlloc(*pcColumns * sizeof(DBCOLUMNINFO));
			if (*prgInfo != NULL)
			{
				...
				*ppStringsBuffer = (OLECHAR*)CoTaskMemAlloc(cwRequired * sizeof(OLECHAR));
				...
				{
					//Cache size
					SIZE_T allocatedSize = cwRequired;
					DBORDINAL iCol;
					size_t iOffset;
					for (iCol = 0, iOffset = 0; iCol < *pcColumns; iCol++)
					{
						if (pInfo[iCol].pwszName)
						{
							ocscpy_s(*ppStringsBuffer + iOffset, allocatedSize - iOffset, pInfo[iCol].pwszName);
	...
	public:
			...
			DBBINDSTATUS rgStatus[], bool bHasBookmarks)
		{
			...
			for (DBCOUNTITEM iBinding = 0; iBinding < cBindings; iBinding++)
			{
				...
				if(pColInfo[iOrdAdjusted].wType == DBTYPE_BYTES)
					hrConvert = S_OK;
				//ISequentialStream ���ε�� ��� Ȯ��
			...
			DBBINDSTATUS rgStatus[])
		{
			...
			*phAccessor = NULL;
			...
			//			//if (FAILED(hr))
			//				//return hr;
			//			pObjStream->m_contained.Initialize();
			//			BYTE* temp = (BYTE*)(0x0012fe40);// + rgBindings[i].obValue;
...
#define END_PROVIDER_COLUMN_MAP() \
	...
	public:
			...
			DBROWSTATUS rgRowStatus[])
		{
			...
			rgRowOptions; //TODO Investigate this
	...
	//http://msdn.microsoft.com/en-us/library/ms721241.aspx
	class CCatalogsRow
	{
	public:
		...
		BEGIN_PROVIDER_COLUMN_MAP(CCatalogsRow)
	...
	class CSQLLanguagesRow
	{
	public:

		WCHAR m_szSQL_LANGUAGE_SOURCE[129];
		WCHAR m_szSQL_LANGUAGE_YEAR[129];
		WCHAR m_szSQL_LANGUAGE_CONFORMANCE[129];
		WCHAR m_szSQL_LANGUAGE_INTEGRITY[129];
		WCHAR m_szSQL_LANGUAGE_IMPLEMENTATION[129];
		WCHAR m_szSQL_LANGUAGE_BINDING_STYLE[129];
		WCHAR m_szSQL_LANGUAGE_PROGRAMMING_LANGUAGE[129];
		...
		BEGIN_PROVIDER_COLUMN_MAP(CSQLLanguagesRow)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_SOURCE", 1, m_szSQL_LANGUAGE_SOURCE)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_YEAR", 2, m_szSQL_LANGUAGE_YEAR)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_CONFORMANCE", 3, m_szSQL_LANGUAGE_CONFORMANCE)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_INTEGRITY", 4, m_szSQL_LANGUAGE_INTEGRITY)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_IMPLEMENTATION", 5, m_szSQL_LANGUAGE_IMPLEMENTATION)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_BINDING_STYLE", 6, m_szSQL_LANGUAGE_BINDING_STYLE)
			PROVIDER_COLUMN_ENTRY_WSTR("SQL_LANGUAGE_PROGRAMMING_LANGUAGE", 7, m_szSQL_LANGUAGE_PROGRAMMING_LANGUAGE)
		END_PROVIDER_COLUMN_MAP()
