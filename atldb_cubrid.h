/*
 * 다음은 Microsoft Visual C++ .NET(7.0)에 포함된 atldb.h를 수정한 파일이다.
 * atldb.h의 버그는 가능하면 override를 통해서 해결했다.
 * 하지만 불가능한 경우와 여러 곳에서 사용되어서(IAccessorImpl 같은 경우)
 * override로는 같은 코드가 중복되는 경우에는 이 파일을 수정했다.
 * 수정된 사항을 알고 싶으면 atldb_orig.h와 비교하면 된다.
 */
/*
 * 1. IRowsetIdentityImpl은 RowClass를 인자로 받는데, 정작
 *   CRowsetBaseImpl은 넘겨주지 않는다. RowClass로 초기값(CSimpleRow)을
 *   사용하지 않기 때문에 문제가 된다.
 *
 * 2. IRowsetLocateImpl::GetRowsByBookmark에서 CreateRow를 호출하는 부분이 있다.
 *   이것이 pT->CreateRow가 아니라서, override한 CreateRow가 호출되는 대신
 *   IRowsetImpl::CreateRow가 호출된다. (CreateRow는 virtual function이 아니다.)
 *   다른 부분의 CreateRow는 전부 pT->가 붙기 때문에 override한 CreateRow가 호출된다.
 *   이를 수정했다.
 *
 * 3. GetNextRows에서 CreateRow시 HROW를 해제하는 명령의 precedence 오류 수정
 *   *prghRows[iRowDel] -> (*prghRows)[iRowDel]
 *
 * 4. Rowset의 변화가능(m_bIsChangeable)을 UPDATABILITY 속성과
 *   IRowsetChange 속성 둘 다를 보고 판단하도록 했다. IRowsetChange가 FALSE여도
 *   UPDATABILITY 속성은 기본값인 CHANGE | INSERT | DELETE 를 가지고 있다.
 *    see IAccessorImpl::CreateAcceessor.
 *
 * 5. ICommandTextImpl::SetCommandText에서 정상적인 property가 아닌 값을
 *   구하려는 것을 수정
 *
 * 6. Column 별 property를 반환할 수 있게 함
 *    1) class CPropColID의 복사 생성자를 만듦
 *    2) 요청된 속성만을 세는 CUtlPropsBase::GetCountofColids 함수 추가
 *    3) CUtlPropsBase::GetProperties에서 전체 속성이 아닌
 *       일부 속성만을 요청했을 경우에도 GetCountofColids 호출
 *    4) CUtlPropsBase::RetrieveColumnIdProps의 버그 수정
 *
 * 7. IAccessorImplBase::ValidateBindings에서 좀 더 많은 validate를 함
 *
 * 8. CUtlPropInfo::GetPropertyInfo
 *    DBINITALL을 요청했을때 USER_INIT_SET도 반환하도록 함
 *    코드로 봤을 때 *ALL을 두가지 이상 요청했을 때 문제가 생길 듯 하나
 *    우선은 놔둠.
 *
 * 9. IOpenRowsetImpl::CreateRowset에서 OnInterfaceRequested 실패시
 *   정확한 코드를 반환하게 함
 */
...
public:
	...
	CPropColID(const CPropColID &colId)
	{
		*this = colId;
	}
	CPropColID &operator =(const CPropColID &colId)
	{
		dwOption = colId.dwOption;
		VariantInit(&vValue);
		VariantCopy(&vValue, const_cast<VARIANT *>(&colId.vValue));
		CopyDBIDs(&dbidProperty, &colId.dbidProperty);
		return *this;
	}
};
						...
						const GUID* pGuid = NULL)
	{
		...
		for(ulSet=0; ulSet<ulEnd; ulSet++)
		{
			...
			else
			{
				...
				{
					if(bInitialized || InlineIsEqualGUID(guidSet, DBPROPSET_DBINITALL))
					{
...
//							pPropInfoSet[ulOutIndex].guidPropertySet	= rgPropertySets[ulSet].guidPropertySet;
					...
					{
						// Initialize가 안 됐다면 GUID에 DBPROPSET_COLUMN 대신
						// DBPROPSET_COLUMNALL을 그대로 넣어서 에러를 발생시킨다.
						pPropInfoSet[ulOutIndex].guidPropertySet = rgPropertySets[ulSet].guidPropertySet;
						pPropInfoSet[ulOutIndex].cPropertyInfos = 0;
						ulOutIndex++;
					}
			...
			else
			{
				...
				if( pPropInfoSet[ulSet].cPropertyInfos == 0 )
				{
					...
					if( (GetPropertySetIndex(&(pPropInfoSet[ulSet].guidPropertySet)) == S_FALSE)
						|| (!bInitialized && 
// USER_INITSET도 받아들이게 함
// GetPropertySetIndex==S_OK면 m_cPropSetDex>0이고,
// guidPropertySet은 *ALL이 아닐테니까,
// m_rgiPropSetDex의 0번 값이 guidPropertySet을 가진 SET이 될 것임.
// 따라서 그 flag를 보면 됨
						!(m_pUPropSet[m_rgiPropSetDex[0]].pUPropInfo[0].dwFlags & DBPROPFLAGS_DBINIT)))
		...
		for(ulSet=0; ulSet<cSets; ulSet++)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						cProps += GetCountofColids(&m_pUProp[ulCurSet], rgPropertySets[ulSet]);
						_ATLDUMPPROPSETIID(pPropSet[iPropSet].guidPropertySet, dwStatus);
					}
				}
			}
			...
			if( pProp )
			{
				...
				if(dwStatus & GETPROP_ALLPROPIDS)
					cPropertyIDs = m_pUPropSet[ulCurSet].cUPropInfo;
				else
					cPropertyIDs = rgPropertySets[ulSet].cPropertyIDs;
				for(ulProp=0; ulProp<cPropertyIDs; ulProp++)
				{
...
NEXT_SET:
	...
	ULONG GetCountofColids(UPROP* pUProp, const DBPROPIDSET &rPropIDSet)
	{
		ULONG	cExtra=0;
		ATLASSERT(pUProp);
		for(ULONG ul=0; ul<pUProp->cPropIds; ul++)
		{
			...
			{
				// 요청된 property일 때만 센다.
				for(ULONG i=0;i<rPropIDSet.cPropertyIDs;i++)
				{
					if(pUProp->rgpUPropInfo[ul]->dwPropId==rPropIDSet.rgPropertyIDs[i])
						// cPropertyIDs를 기본값으로 가지므로 컬럼수를 다 더하면
						// 배열 하나가 남는다.
						// TODO: 위의 함수도 수정? (단지 메모리를 넉넉하게 잡을 뿐이므로
						// 문제는 없다)
						cExtra += (pUProp->pUPropVal[ul].pCColumnIds)->GetCountOfPropColids() - 1;
				}
			}
		}
		return cExtra;
	...
	void RetrieveColumnIdProps(DBPROP rgProp[], UPROPVAL* pUPropVal, ULONG* pulNext)
	{
		// Reset to first Node
		CColumnIds* pColIds = pUPropVal->pCColumnIds;
		HRESULT hr = E_FAIL;
		DBPROPID dwPropertyID = rgProp[*pulNext].dwPropertyID;
		for (size_t i = 0; i < pColIds->GetCount(); i++)
		{
			DBPROP *pCurProp = &rgProp[*pulNext];
			pCurProp->dwStatus = DBSTATUS_S_OK;
			pCurProp->dwPropertyID = dwPropertyID;
...
public:
						 ...
						 RowsetClass*& pRowsetObj)
	{
		...
		// requested.
		if (FAILED(hr = pRowsetObj->OnInterfaceRequested(riid)))
			return hr;
...
public:
	...
	STDMETHOD(MapColumnIDs)(DBORDINAL cColumnIDs,
							...
							DBORDINAL rgColumns[])
	{
		// CUBRIDProvider의 Column ID는 DBKIND_GUID_NAME이다.
		// rgColumnIDs에 DBKIND_NAME이 들어와도 찾을 수 있도록
		// IColumnsInfoImpl::MapColumnIDs의 내용을 복사 수정했다.
		// 또한 NAME도 대소문자를 구별하지 않도록 했다.
		...
		for (DBORDINAL iColId = 0; iColId < cColumnIDs; iColId++)
		{
			...
			while(iColMapCur < cCols && !bDone)
			{
				// modify begin
				DBID tmp;
				CopyDBIDs(&tmp, &(rgColumnIDs[iColId]));
				if(pInfo[iColMapCur].columnid.eKind==DBKIND_GUID_NAME)
				{	// CCUBRIDRowset에 대해서만 적용. Schema Rowset은 DBKIND_NAME 사용
					if(tmp.eKind==DBKIND_NAME)
					{
						tmp.eKind = DBKIND_GUID_NAME;
						memset(&tmp.uGuid.guid, 0, sizeof(GUID));
						tmp.uGuid.guid.Data1 = pInfo[iColMapCur].columnid.uGuid.guid.Data1;
					}
					if(DBID_USE_NAME(tmp.eKind))
						_wcsupr(tmp.uName.pwszName);
				}
				hr = CompareDBIDs(&(pInfo[iColMapCur].columnid), &tmp);
				FreeDBIDs(&tmp);
				// modify end
...
public:
	...
	STDMETHOD(SetCommandText)(REFGUID rguidDialect,LPCOLESTR pwszCommand)
	{
		...
		if (InlineIsEqualGUID(rguidDialect, DBGUID_SQL))
		{
			...
			HRESULT hr = pT->GetSite(__uuidof(IGetDataSource), (void **)&spGetDataSource);
			ATLASSERT(hr==S_OK);

			CComPtr<IDBProperties> spDBProperties;
			hr = spGetDataSource->GetDataSource(__uuidof(IDBProperties), (IUnknown **)&spDBProperties);
			ATLASSERT(hr==S_OK);

			CDBPropIDSet tmp(DBPROPSET_DATASOURCEINFO);
			tmp.AddPropertyID(DBPROP_SQLSUPPORT);

			ULONG cPropSet = 0;
			CComHeapPtr<DBPROPSET> rgPropSet;
			hr = spDBProperties->GetProperties(1, &tmp, &cPropSet, &rgPropSet);
			ATLASSERT(hr==S_OK);

			CComVariant varByRef = rgPropSet->rgProperties[0].vValue;
			VariantClear(&(rgPropSet->rgProperties[0].vValue));
			CoTaskMemFree(rgPropSet->rgProperties);

			if(V_I4(&varByRef) & DBPROPVAL_SQL_NONE)
...
public:
	...
	HRESULT ValidateBindings(DBCOUNTITEM cBindings, const DBBINDING rgBindings[], 
				...
				DBACCESSORFLAGS dwAccessorFlags, DBLENGTH cbRowSize)
	{
		HRESULT hr = S_OK;

		for (ULONG iBinding = 0; iBinding < cBindings; iBinding++)
		{
			rgStatus[iBinding] = DBBINDSTATUS_OK;
			...
			{
				// param accessor에 bookmark ordinal은 오류
				if (rBindCur.iOrdinal == 0)
				{
					hr = DB_E_ERRORSOCCURRED;
					rgStatus[iBinding] = DBBINDSTATUS_BADORDINAL;
				...
				// 잘못된 eParamIO
				// output parameter 지원 안 함
				// param accessor에 provider 할당 메모리는 오류
				if (rBindCur.eParamIO==DBPARAMIO_NOTPARAM ||
					HasFlag(rBindCur.eParamIO, DBPARAMIO_OUTPUT) ||
					rBindCur.dwMemOwner==DBMEMOWNER_PROVIDEROWNED)
				{
					hr = DB_E_ERRORSOCCURRED;
					rgStatus[iBinding] = DBBINDSTATUS_BADBINDINFO;
					continue;
				...
				// 같은 ordinal이 있으면 오류
				for(ULONG j=0;j<iBinding;j++)
				{
					if(rBindCur.iOrdinal==rgBindings[j].iOrdinal)
					{
						hr = DB_E_ERRORSOCCURRED;
						rgStatus[iBinding] = DBBINDSTATUS_BADBINDINFO;
						break;
				...
				if(rgStatus[iBinding]!=DBBINDSTATUS_OK)
					continue;
			}
			...
			// 다른 컬럼의 offset과의 겹칩도 검사하지 않는다.
			if((rBindCur.dwPart & DBPART_VALUE) && (rBindCur.wType==DBTYPE_STR ||
				rBindCur.wType==DBTYPE_WSTR || rBindCur.wType==DBTYPE_BYTES))
			{
				if( ( (rBindCur.dwPart & DBPART_STATUS) &&
					  rBindCur.obValue<=rBindCur.obStatus &&
					  rBindCur.obStatus<rBindCur.obValue+rBindCur.cbMaxLen ) ||
					( (rBindCur.dwPart & DBPART_LENGTH) &&
					  rBindCur.obValue<=rBindCur.obLength &&
					  rBindCur.obLength<rBindCur.obValue+rBindCur.cbMaxLen ) )
				{
					hr = DB_E_ERRORSOCCURRED;
					rgStatus[iBinding] = DBBINDSTATUS_BADBINDINFO;
					break;
...
public:
				...
				DBBINDSTATUS rgStatus[], bool bHasBookmarks)
	{
		...
		for (DBCOUNTITEM iBinding = 0; iBinding < cBindings; iBinding++)
		{
			...
			//ISequentialStream 바인딩을 위해 확장
			if (hrConvert == S_FALSE && rBindCur.wType == DBTYPE_IUNKNOWN)
			{
				if (rBindCur.pObject)
				{
					if (rBindCur.pObject->iid != IID_ISequentialStream)
						return E_NOINTERFACE;
							  ...
							  DBBINDSTATUS rgStatus[])
	{
		...
		*phAccessor = NULL;
		...
		hr = ValidateBindings(cBindings, rgBindings, rgStatus, bHasBookmarks, dwAccessorFlags, cbRowSize);
		if (FAILED(hr))
			return hr;
		if (!m_bIsCommand)
		{
...
//				lRow = ((long)*rgpBookmarks[l]) - 1;
			...
			if (pT->CreateRow(lRow, ulRowsObtained, &rghRows[l]) != S_OK)
			{
				bErrors |= true;
			}
			else
...
class CRowsetImpl : 
	...
	public CRowsetBaseImpl<T, Storage, ArrayType, RowClass, RowsetInterface>,
